...
export default function flexibleCustomTagsPlugin(md) {
  ...
  function customTagRule(state, startLine, endLine, silent) {
    ...
    for (const [tagName, tagConfig] of Object.entries(tags)) {
      if (state.src.startsWith(tagConfig.start, start)) {
        ...
        if (!silent) {
          ...
          let token = state.push('custom_tag_open', 'div', 1)
          token.block = true
          token.meta = { tagName }

          token = state.push('custom_tag_content', '', 0)
          token.content = content
          token.meta = { tagName } // 添加 meta 到 content token

          token = state.push('custom_tag_close', 'div', -1)
          token.block = true
          token.meta = { tagName } // 添加 meta 到 close token
        }

        state.line = state.src.slice(contentEnd + endMarker.length).split('\n').length + startLine
  ...
  md.block.ruler.before('paragraph', 'custom_tag', customTagRule)

  // 渲染自定义标签
  md.renderer.rules.custom_tag_open = function (tokens, idx) {
    const token = tokens[idx]
    const tagName = token.meta && token.meta.tagName ? token.meta.tagName : 'default'
    return `<p><${tagName === 'custom' ? 'hisw-echarts' : 'div'} class="${tagName}-tag">`
  }

  md.renderer.rules.custom_tag_content = function (tokens, idx) {
    const token = tokens[idx]
    const tagName = token.meta && token.meta.tagName ? token.meta.tagName : 'default'
    const renderFn = tags[tagName] ? tags[tagName].render : content => content
    return renderFn(token.content)
  }

  md.renderer.rules.custom_tag_close = function (tokens, idx) {
    const token = tokens[idx]
    const tagName = token.meta && token.meta.tagName ? token.meta.tagName : 'default'
    return `</${tagName === 'custom' ? 'hisw-echarts' : 'div'}></p>`
  }
